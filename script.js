...
function countryOptions(value){
  const current=COUNTRY_FLAGS.some(([flag])=>flag===value)?value:"🇮🇩";
  return COUNTRY_FLAGS.map(([flag,name])=>`<option value="${flag}" ${current===flag?"selected":""}>${flag} ${escapeHtml(name)}</option>`).join("");
}
function countrySelect(value,onchange,title="Pilih negara"){
  return `<select class="country-select" title="${escapeAttr(title)}" onchange="${onchange}">${countryOptions(value)}</select>`;
}
...
function renderTeams(){
  ...
  <div class="card team-card">
    ...
    <div class="field"><label>NAMA TIM</label><input value="${escapeAttr(t.name)}" onchange="updateTeam(${t.id},this.value)"></div><div class="player-list"><div class="field"><label>TAG TIM</label><input value="${escapeAttr(t.tag)}" onchange="updateTeamField(${t.id},'tag',this.value)"></div><div class="field"><label>BENDERA TIM</label>${countrySelect(t.flag,`updateTeamField(${t.id},'flag',this.value)`,`Bendera tim`)}</div></div>
    <h4 class="subheading">Pemain Utama</h4>
    <div class="player-list">${t.players.map((p,i)=>`<div class="field"><label>PEMAIN ${i+1}</label><input value="${escapeAttr(p)}" onchange="updatePlayer(${t.id},${i},this.value)">${countrySelect(t.playerFlags[i],`updatePlayerFlag(${t.id},${i},this.value)`,`Bendera pemain ${i+1}`)}</div>`).join("")}</div>
    <h4 class="subheading">Pemain Cadangan</h4>
    <div class="player-list">${t.reserves.map((p,i)=>`<div class="field"><label>CADANGAN ${i+1}</label><input value="${escapeAttr(p)}" onchange="updateReserve(${t.id},${i},this.value)">${countrySelect(t.reserveFlags[i],`updateReserveFlag(${t.id},${i},this.value)`,`Bendera cadangan ${i+1}`)}</div>`).join("")}</div>
